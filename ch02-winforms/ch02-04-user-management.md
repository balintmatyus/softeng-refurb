...
1. Hozz létre egy új Windows Forms űrlapot `UgyfelListaForm` néven.
2. Tervezd meg az űrlapot a következő vezérlőkkel:
...
3. Hozz létre egy privát mezőt a `DbContext` példánynak:
   ```csharp
   private RendelesDbContext _context;
   ```

4. Inicializáld a `DbContext`-et az űrlap konstruktorában:
   ```csharp
...
5. Állítsd be az adatforrást a `DataGridView`-hoz a tervező nézetben:
...
6. Állítsd be az adatkötést a kódban:
   ```csharp
...
7. Implementáld a keresőmező TextChanged eseménykezelőjét. A cél az, hogy a felhasználó által beírt szöveg alapján szűrjük az ügyfelek listáját. Ehhez a következő lépéseket érdemes követni:
...
## 5. Az UgyfelSzerkesztesForm implementálása

11. A korábban létrehozott `UgyfelSzerkesztesForm`-on helyezd el a következő vezérlőket:
...
12. Implementáld az adatkötést az `UgyfelSzerkesztesForm`-on:
    ```csharp
    ...
    a) Rugalmasság: A konstruktor paramétere opcionális (`Ugyfel ugyfel = null`), ami lehetővé teszi, hogy ugyanazt az űrlapot használjuk új ügyfél létrehozására és meglévő ügyfél szerkesztésére is.

    b) Null-coalescing operátor használata: A `this.SzerkesztettÜgyfél = ugyfel ?? new Ugyfel();` sor biztosítja, hogy mindig legyen egy érvényes `Ugyfel` objektumunk. Ha `null`-t kap paraméterként (új ügyfél esetén), akkor egy új `Ugyfel` példányt hoz létre.
...
13. Állítsd be a név TextBox Validating eseménykezelőjét:
    ```csharp
...
14. Hasonló módon implementáld a validációt az email és telefonszám mezőkhöz is, a fent leírt követelmények alapján.

15. Végül, a mentés gomb Click eseménykezelőjében ellenőrizd, hogy az űrlap érvényes-e:
    ```csharp
...
## Házi feladat: Címek kezelésének implementálása
